...
2. **Read each page component** referenced by the route, then follow imports into its section/child components. Read enough to extract:
   ...
   - Section order as rendered top→bottom (mirror JSX order).
...
- If `WIREFRAME.md` **does** exist → ask the user, verbatim:
  > WIREFRAME.md já existe. Deseja fazer **overwrite** ou salvar como nova versão (**WIREFRAME-YYYY-MM-DD_HH-MM.md**)?
  Then act on their answer:
  - "overwrite" → overwrite `WIREFRAME.md`.
  - "nova versão" / "new version" → write to `WIREFRAME-YYYY-MM-DD_HH-MM.md` using the current local date/time (24h).
...
### 2.1 `/` — Index
ASCII layout box mirroring top→bottom section order:
...
- **ASCII boxes reflect true visual order** (top→bottom as rendered, not import order).
