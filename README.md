...
Run `/jevify` to audit runtime AI calls in your Lovable project. It returns an inventory, candidates for JEV, risks, and next steps in chat. It does not change code or reduce build credits.

Run `/jevify migrate <finding>` to move one candidate to JEV. It explains the plan, changes code only after you approve it, and adds the JEV decision next to the existing AI call behind an `off | shadow | on` flag that starts in `shadow`. Optionally add the jevify MCP server (`https://jevify.lovable.app/mcp`) under **Connectors**; without it, the skill runs locally.
